...
class TestPDFSplitterService(unittest.TestCase):
    ...
    def test_init_without_repository(self):
        with patch("src.services.pdf_splitter_service.PDFRepository") as MockRepo:
    ...
    def test_init_with_repository(self):
        with patch("src.services.pdf_splitter_service.PDFRepository") as MockRepoCheck:
    ...
    def test_split_by_fixed_range_success(self, mock_isdir):
        ...
        filename_base = os.path.splitext(os.path.basename(self.test_file_path))[0]
        expected_paths = [
            os.path.join(self.output_dir, f"{filename_base}_sivut_1-3.pdf"),
            os.path.join(self.output_dir, f"{filename_base}_sivut_4-6.pdf"),
            os.path.join(self.output_dir, f"{filename_base}_sivut_7-7.pdf"),
        ]
    ...
    def test_split_by_fixed_range_single_file(self, mock_isdir):
        ...
        filename_base = os.path.splitext(os.path.basename(self.test_file_path))[0]
        expected_path = os.path.join(self.output_dir, f"{filename_base}_sivut_1-10.pdf")
    ...
    def test_split_by_custom_ranges_success(self, mock_isdir):
        ...
        filename_base = os.path.splitext(os.path.basename(self.test_file_path))[0]
        expected_paths = [
            os.path.join(self.output_dir, f"{filename_base}_alue_1_sivut_1-3.pdf"),
            os.path.join(self.output_dir, f"{filename_base}_alue_2_sivut_5-5.pdf"),
            os.path.join(self.output_dir, f"{filename_base}_alue_3_sivut_8-10.pdf"),
        ]
        ...
        self.assertEqual(
            self.mock_repository.extract_pages.call_args_list,
            expected_extract_calls,
        )
        expected_save_calls = [
            call(mock_doc1, expected_paths[0]),
